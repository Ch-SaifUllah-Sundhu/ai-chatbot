...
const { connectDB, Conversation } = require("./db");
...
app.post("/api/chat", verifyToken, upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    let conv = await Conversation.findOne({ userId, conversationId });
    let history = conv ? conv.history : [];
    ...
    if (conv) {
      ...
      await Conversation.create({ userId, conversationId, history });
    }
...
app.delete("/api/chat/:conversationId", verifyToken, async (req, res) => {
  const userId = req.user.id;
  try {
    await Conversation.deleteOne({ userId, conversationId: req.params.conversationId });
